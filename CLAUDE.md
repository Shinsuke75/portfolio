...
- **コーデックス（外部AI）レビュー対応（2026-07-02）**
  - スマホ横はみ出し修正：hero-eyebrow（英字ラベル・nowrap）が376px幅で410pxに広がりページを466pxにしていた。600px以下で改行許可＋hero-name縮小＋overflow-x:clip＋全要素min-width:0。360〜900pxで実測確認済み
  - 背景を生成り（--bg:#f3f0e8 / --bg2:#ece8dc）に変更。明朝・茶差し色は見送り（明朝は過去に不採用の経緯）
  - Aboutスキルタグから「生成AI」を削除（唐突との指摘。About本文の記述は維持）
  - フォームの「GET送信では?」という指摘は誤検出（実際はAJAX+FormSubmit、6/30実機テスト済み）
  - キャッチコピー変更案（「生木を削り、道具をつくり、ものづくりを教えています。」）は保留→「越境」コンセプトをコーデックスに伝えて相談する方針。相談用プロンプトをユーザーに渡した
