...
- **2ボタンUI実装済み**（「📷 カメラで撮影」「🖼 写真を選ぶ」を別ボタンに分離）
- **サムネイルプレビュー追加**（写真選択後にボタン下に縮小表示）
- **deploy.bat 作成済み**（`C:\Users\katou\OneDrive\Desktop\mokusan-fix\deploy.bat` をダブルクリックで commit & push & Vercel自動デプロイ）
- **フェーズ2：「データ・計算」タブ追加済み**（コード適用済み・Vercelデプロイ待ち確認中）
  - `api/get-prices.js`：Google Sheetsからデータ読み取り・集計APIを新規作成
  - タブ切り替えUI（スキャン ↔ データ・計算）
  - 樹種・都道府県で絞り込み → 平均立米単価を自動表示・計算欄に自動入力
  - 積算計算：単価×寸法×本数 → 金額計算
  - みんなの記録：最新20件をテーブル表示

### データ設計方針
- 全ユーザーのデータが1つのシートに集約（コミュニティデータ収集が目的）
- データ読み取りはフェーズ2のタブから誰でも閲覧可能
...
- [ ] スマホで https://mokusan.vercel.app を開いてタブ表示・データ表示を動作確認
- [ ] アプリ説明文の更新（「データを共有して全国の木材価格を集める」旨を追加）
- [ ] 材料リスト機能（複数材を追加して合計金額を出す）← フェーズ3候補
